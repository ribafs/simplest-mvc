    ...
    // Caso o controller da url exista na pasta App, então crie uma instância do mesmo
        ...
        if (method_exists($this->urlController, $this->urlAction) && is_callable(array($this->urlController, $this->urlAction))) {                
            ...
            if (strlen($this->urlAction) == 0) { 
                ...
                $page->index(1); // 1 - action , 2 - controller
            }
        }
...
Esta é uma classe muito importante no aplicativo, controlando a execução dos controllers e actions.
Quando o usuário entra na url um endereço para controller, action e parâmetro que não existe ele receberá uma mensagem amigável.
